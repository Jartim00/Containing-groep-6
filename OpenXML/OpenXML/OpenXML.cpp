...
bool checkTime(string time){
	...
	int temp = time.find('.');
	if (atoi(time.substr(0, temp).c_str()) >= 12) return false;
	if (atoi(time.substr(temp + 1).c_str()) >= 60) return false;
	return true;
}
...
{
	int aankomstdag = atoi(split(a).c_str()); int aankomstmaand = atoi(split(b).c_str()); int aankomstjaar = atoi(split(c).c_str());
	int vertrekdag = atoi(split(d).c_str()); int vertrekmaand = atoi(split(e).c_str()); int vertrekjaar = atoi(split(f).c_str());
	...
	if (aankomstjaar = vertrekjaar)
	{
		...
		if (aankomstmaand = vertrekmaand)
		{
			...
			{
				int temp = g.find('.');
				int aankomsttijd = atoi(g.substr(0, temp).c_str());
				int vertrektijd = atoi(h.substr(temp + 1).c_str());
				if (atoi(g.substr(0, temp).c_str()) > atoi(h.substr(temp + 1).c_str())) return false;
				else return true;
			}
		}
	}
			...
			if (iss >> sub)
			{
				...
				if (sub.find("<tot>") != string::npos)
				{
					...
					else
					{
						...
						{
							checkData(container["aankomstdag"], container["aankomstmaand"], container["aankomstjaar"], container["vertrekdag"], container["vertrekmaand"], container["vertrekjaar"], container["aankomsteindtijd"], tempvan);
				...
				if (sub.find("<inhoud>") != string::npos)
